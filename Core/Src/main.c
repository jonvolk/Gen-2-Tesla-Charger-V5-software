...
int main(void)
{
  ...
  if (parameters.version != EEPROM_VERSION)
  {
	  ...
	  parameters.currReq = (43 * 1500); //max input limit per module 1500 = 1A
...
void enabledCAN(){
	if (bChargerEnabled)
		      {
		myTxMessage.StdId = 0x2D0;            // Set our transmission address ID
...
void ACcurrentlimit()
{
  if (parameters.autoEnableCharger == 1)
  {
    ...
    if (parameters.phaseconfig == 0)
    {
      modulelimcur = (accurlim / 3) * 2 ; // all module parallel, sharing AC input current.....value was 1.5, 2 is allowing 40a of charge current
    }
    else
    {
      modulelimcur = accurlim * 2; // one module per phase, EVSE current limit is per phase...value was 1.5, 2 is allowing 4a of charge current
